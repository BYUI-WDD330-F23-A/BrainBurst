...
function options(correctAnswer, badAnswers) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const theAnswer = i === goodIndex ? correctAnswer : badAnswers[i];
    const resultFunction = i === goodIndex ? imTrue : imFalse;
...
function updateOptions(correctAnswer, badAnswers) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const inputElement = document.getElementById(`option${i}`);
